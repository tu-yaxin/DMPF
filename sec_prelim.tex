...
\Yaxin{Asymptotic parameters: \cite[Theorem 1]{cryptoeprint:2022/1455} gives $w$-way cuckoo hashing scheme for $t$ balls, with $w = O(\sqrt{\lambda_{\stat}\log t})$ and $m = O(t)$, making $O(\sqrt{\lambda_\stat \log n})$ queries to the hash functions. It supposes the hash functions from a $O(n\sqrt{\lambda_\stat \log n})$-wise independent hash function family}
