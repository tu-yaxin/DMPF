...
\Yaxin{Asymptotic parameters: 

On one extreme of linear buckets and many hash functions: \cite[Theorem 1]{yeo_cuckoo_2023} gives $w$-way cuckoo hashing scheme for $t$ balls, with $w = O(\sqrt{\lambda_{\stat}\log t})$ and $m = O(t)$, making $O(\sqrt{\lambda_\stat \log t})$ queries to the hash functions. It supposes the hash functions from a $O(t\sqrt{\lambda_\stat \log t})$-wise independent hash function family. 

On the other extreme of 3 hash functions and quasi-linear buckets: \cite[Remark 1]{cryptoeprint:2021/580} gives $w=3$ and $m = O(\lambda_\stat t + t\log t)$, while supposing the hash functions are collision resilient. It also claims empirical results that $\lambda=80$, $w=3$, $m\le 2t$ for $30\le t\le 2^{37}$. }

\paragraph{The failure probability of cuckoo hashing.}Let's denote the failure probability of $w$-way cuckoo hashing to be $\epsilon=2^{-\lambda_{\stat}}$. In practice we usually consider the statistical security parameter $\lambda_{\stat}$ to be $30$ or $40$. The empirical result in \cite{chen_fast_2017} shows for $w=3$, $m=16384$, $\lambda_\stat = 124.4e-144.6$ where $e=m/t$ is the expansion parameter. For $w=3$, $m=8192$, $\lambda_\stat=125e-145$. However we use cuckoo hashing to construct DMPF for $t$-point functions, in which case we'd also care about $t$ being small, say $2,3$ or $100$, and $m$ should not be too large. In this sense the previous empirical results are not complete. \Yaxin{\cite{cryptoeprint:2017/1142} uses $w=3, e=1.5, t>200$ and $\lambda_\stat\approx 40$ and claims it follows the analysis from \cite{chen_fast_2017}, but I don't see how... It also claims $\lambda_\stat\approx 20$ for small $k$, but not sure how small can $k$ be. }
